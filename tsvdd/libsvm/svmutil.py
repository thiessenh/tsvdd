...
def svm_predict(x, gram_diag, m):
	...
	if predict_probability:
		...
		if svm_type in (ONE_CLASS, EPSILON_SVR, NU_SVC, SVDD):
			nr_classifier = 1
		else:
			nr_classifier = nr_class*(nr_class-1)//2
		dec_values = (c_double * nr_classifier)()
		if len(x) != len(gram_diag):
			raise ValueError(f'Predict needs diagonal of gram matrix for `testing` vector. Length X != Length gradm_diag, {len(x) } != len(gram_diag)')
		for xi, k_xx in zip(x, gram_diag):
			k_xx_c = (c_double)()
			k_xx_c = k_xx
			xi, idx = gen_svm_nodearray(xi, isKernel=(m.param.kernel_type == PRECOMPUTED))
			label = libsvm.svm_predict_values(m, xi, dec_values, k_xx_c)
			if(nr_class == 1):
				values = [1]
			else:
