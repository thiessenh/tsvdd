...
def train_kernel_matrix(np.ndarray[np.double_t,ndim=3] seq, double sigma, int triangular):
  """ Compute the Triangular Global Alignment (TGA) similarity score
  ...
  trainGramMatrix(<double*> seq.data, <int> nInstances, <int> nLength, <int> nDim, <double*> res.data, <double> sigma, <int> triangular)


  return np.exp(-res)

def test_kernel_matrix(np.ndarray[np.double_t, ndim=3] train, np.ndarray[np.double_t, ndim=3] test, double sigma, int triangular):
  """ Compute the Triangular Global Alignment (TGA) similarity score
  ...
  testGramMatrix(<double*> train.data, <double*> test.data, <int> nInstances_train, <int> nInstances_test, <int> nLength_train, <int> nLength_test, <int> nDim, <double*> res.data, <double> sigma,
               <int> triangular)
