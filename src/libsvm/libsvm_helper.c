 ...
 *
 */
struct svm_node *dense_to_libsvm (double *x, npy_intp *dims, int precomputed)
{
...
void set_problem(struct svm_problem *problem, char *X, char *Y, char *sample_weight, npy_intp *dims, int kernel_type)
{
    ...
    if (kernel_type == 4)
        precomputed = 1;
    problem->x = dense_to_libsvm((double *) X, dims, precomputed); /* implicit call to malloc */
                            ...
                            char *probA, char *probB)
{
    ...
    if (param->kernel_type == PRECOMPUTED) {
        if ((model->SV = malloc ((model->l) * sizeof(struct svm_node))) == NULL)
            goto SV_error;
        for (i=0; i<model->l; ++i) {
            // libsvm starts indexing with 1
            int sv_index = ((int *) support)[i] - 1;
            double *point_sv_index = (double*) malloc(sizeof(double) * 1);
            point_sv_index[0]=(double) sv_index;
            model->SV[i].dim = sv_index;
            model->SV[i].values=point_sv_index;
        }
    } else {
        model->SV = dense_to_libsvm((double *) SV, SV_dims, precomputed);
    }
    /*
 ...
 * Get rho from model.
 */
double get_rho(struct svm_model *model)
{
    return (double) model->rho[0];
}

/*
 * Get r_square from model.
 */
double get_r_square(struct svm_model *model)
{
    return (double) model->r_square;
}
...
int copy_predict(char *predict, struct svm_model *model, npy_intp *predict_dims,
                 char *dec_values, double *K_xx_s)
{
    double *t = (double *) dec_values;
    struct svm_node *predict_nodes;
    npy_intp i;
    int precomputed = 0;
    if (model->param.kernel_type == 4)
        precomputed = 1;

    predict_nodes = dense_to_libsvm((double *) predict, predict_dims, precomputed);
    if (predict_nodes == NULL)
        return -1;
    for(i=0; i<predict_dims[0]; ++i) {
        *t = svm_predict(model, &predict_nodes[i], (double)K_xx_s[i]);
        ++t;
    }
...
int copy_predict_values(char *predict, struct svm_model *model,
                        npy_intp *predict_dims, double *dec_values, int nr_class, double *K_xx_s)
{
    npy_intp i;
    struct svm_node *predict_nodes;
    int precomputed = 0;
    ...
    for(i=0; i<predict_dims[0]; ++i) {
        svm_predict_values(model, &predict_nodes[i],
                                ((double *) dec_values) + i*nr_class, (double) K_xx_s[i]);
    }
...
int free_model(struct svm_model *model)
{
    ...
    /* We don't free sv_indices, since we did not create them in
       set_model */
