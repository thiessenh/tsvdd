	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	for(i=0;i<l;i++)
	{
		alpha[i] = 0;
		minus_ones[i] = -1;
		if(prob->y[i] > 0) {y[i] = +1; C[i] = prob->W[i]*Cp;}else {y[i] = -1;C[i] = prob->W[i]*Cn;}
	}
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	if(n<prob->l)
	    // instance weights
		alpha[n] = prob->W[i] * param->nu * prob->l - n;
	for(i=n+1;i<l;i++)
		alpha[i] = 0;
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	schar *y = new schar[2*l];
	int i;
	double *Cs = new double[2*l];

	double sum = 0;
	for(i=0;i<l;i++)
	{
		Cs[i] = Cs[i+l] = prob->W[i]*param->C;
		sum += Cs[i] * param->nu;
	}
	sum /= 2;
	...
	for(i=0;i<l;i++)
	{
		alpha2[i] = alpha2[i+l] = min(sum,Cs[i]);
		sum -= alpha2[i];
	...
	Solver_NU s;
	s.Solve(2*l, SVR_Q(*prob,*param), linear_term, y,
		alpha2, Cs, param->eps, si, param->shrinking);
	...
	double *alpha, Solver::SolutionInfo* si)
{
	...
	int i,j;
	double r_square;
    // conventional C, necessary to distinguish between valid C (1/l < C < 1) and invalid C
	double C = param->C;
    // instance weights
	...
	double Cp, double Cn)
{
	...
	// might be actually 2*l for some
