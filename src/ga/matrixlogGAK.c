...
void trainGramMatrix(double *seq, int nInstances, int nLength, int nDim, double *res, double sigma, int triangular)
{
    ...
    for(i = 0; i < nInstances; i++){
        ...
        for(j = i; j < nInstances; j++){
            ...
            double ga12 = logGAK((double*) &seq[seq_i], (double*) &seq[seq_j], nLength, nLength, nDim, sigma, triangular);
...
void testGramMatrix(double *train, double *test, int nInstances_train, int nInstances_test, int nLength_train, int nLength_test,int nDim, double *res, double sigma, int triangular)
{
    ...
    for(i = 0; i < nInstances_train; i++){
        int seq_i = seqOffset_train(i);
        cache_train[i] = logGAK((double*) &train[seq_i], (double*) &train[seq_i], nLength_train, nLength_train, nDim, sigma, triangular);
    }
    #pragma omp parallel for private(i)
    for(i = 0; i < nInstances_test; i++){
        int seq_i = seqOffset_test(i);
        cache_test[i] = logGAK((double*) &test[seq_i], (double*) &test[seq_i], nLength_test, nLength_test, nDim, sigma, triangular);
    }
    ...
    for(i = 0; i < nInstances_test; i++){
        for(j = 0; j < nInstances_train; j++){
            ...
            // compute the global alignment kernel value
            double ga12 = logGAK((double*) &train[seq_train], (double*) &test[seq_test], nLength_train, nLength_test, nDim, sigma, triangular);
            // compute the normalization factor
